...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const generateDescription = async (index) => {
        ...
        try {
            const {data} = await api.post('/ai/enhance-job-des', {userContent: prompt}, {headers: {Authorization: token}})
    ...
    {data.length === 0 ? (
        ...
        <div className='space-y-4'>
            {data.map((experience, index)=>(
                <div key={index} className='p-4 border borer-gray-200 rounded-lg space-y-3'>
                    <div className='flex justify-between items-start'>
                        ...
                        <div className='space-y-2'>
                            <div className='flex items-center justify-between'>
                                ...
                                <button onClick={()=> generateDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-2 px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200transition-colors disabled:opacity-50'>
